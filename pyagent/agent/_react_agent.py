...
class ReActTracker:
    ...
    def record_thought(self, thought: str):
        self.thoughts.append({"iteration": self.iteration, "content": thought})
    ...
    def record_observation(self, results: List[ChatResponse]):
        ...
        for r in results:
            if r.tool_result:
                observations.append(
                    {"tool": r.tool_result.fn_name, "output": r.tool_result.fn_output}
                )
        self.observations.append({"iteration": self.iteration, "results": observations})
...
class ClassicReActAgent(BaseAgent):
    ...
    async def reply(
        ...
        for iteration in range(self.max_iterations):
            ...
            self.react_history.append({"iteration": iteration, "response": content})
            ...
            if "<FinalAnswer>" in content:
                if iteration == 0 and self.tools and len(self.tools.list_tools()) > 0:
                    self.logger.warning(
                        "Model tried to answer without using tools, forcing tool use"
                    )
                    force_msg = ChatResponse(role="user", content=get_error_no_tool_prompt())
            ...
            if action and action_input:
                ...
                observation_msg = ChatResponse(role="user", content=get_observation_prompt(output))
                ...
                if iteration == 0 and self.tools and len(self.tools.list_tools()) > 0:
                    error_msg = ChatResponse(role="user", content=get_error_invalid_format_prompt())
                    self.memory.add(error_msg)
                    continue
                else:
                    final_response = ChatResponse(role="assistant", content=content)
                    final_response = self._run_post_reply_hooks(final_response, user_message)
                    yield final_response
                    break
...
if __name__ == "__main__":
    ...
    async def test_react_agent():
        ...
        test_queries = ["现在几点了？", "What is 123 * 456?", "搜索一下人工智能的最新进展"]
    ...
    async def test_classic_react_agent():
        ...
        test_queries = ["现在几点？", "Calculate 999 + 1"]

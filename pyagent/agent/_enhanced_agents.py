...
class PlanReActAgent(Agent):
    ...
    def _extract_final_answer(self, text: str) -> Optional[str]:
        answer_match = re.search(r'<FinalAnswer>(.*?)</FinalAnswer>', text, re.DOTALL | re.IGNORECASE)
        if answer_match:
...
class ReflectionAgent(Agent):
    ...
    def _extract_reflection(self, text: str) -> Optional[str]:
        match = re.search(r'<SelfReflection>(.*?)</SelfReflection>', text, re.DOTALL | re.IGNORECASE)
        if match:
            return match.group(1).strip()
        return None
    
    def _extract_improved_answer(self, text: str) -> Optional[str]:
        match = re.search(r'<ImprovedAnswer>(.*?)</ImprovedAnswer>', text, re.DOTALL | re.IGNORECASE)
        if match:
            return match.group(1).strip()
        return None

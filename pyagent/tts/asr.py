    ...
    def _create_retry_decorator(self):
        self.retry_decorator = retry(
            ...
            wait=wait_exponential(
                min=self.client_cfg.retry_min_wait,
                max=self.client_cfg.retry_max_wait
            ),
            reraise=True,
        )
...
class GummyASR:
    ...
    def recognize_file(self, audio_file: str) -> Dict[str, Any]:
        ...
        return {
            "transcriptions": callback.transcriptions,
...
class BatchTranscriptionASR:
    def __init__(
        self,
        client_cfg: ClientConfig,
        batch_cfg: BatchTranscriptionConfig
    ):
        self.client_cfg = client_cfg
...
class Qwen3ASR:
    ...
    def recognize_file(self, audio_file: str) -> List[str]:
        ...
        self.conversation = OmniRealtimeConversation(
            model=self.qwen3_cfg.model,
            url=self.qwen3_cfg.url,
            callback=callback
        )

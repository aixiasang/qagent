...
class AgenticMemoryAgent(Agent):
    ...
    async def _decide_evolution(
        self, new_note: AgenticMemoryNote, neighbors: List[AgenticMemoryNote]
    ) -> Dict:
        if not neighbors:
            return {
                "should_evolve": False,
    ...
    async def _process_memory(self, note: AgenticMemoryNote) -> bool:
        ...
        try:
            ...
            neighbors = [
                self.agentic_memories[nid]
                for nid, _ in neighbor_ids
                if nid in self.agentic_memories
            ]
    ...
    async def search_memory(
        self, query: str, k: int = 5, include_neighbors: bool = True
    ) -> List[Dict[str, Any]]:
        if not self.agentic_memories:
            return []
                ...
                for memory_dict in list(memories):
                    ...
                    if memory_obj and memory_obj.links:
                        for link_id in memory_obj.links:
                            if (
                                link_id not in seen_ids
                                and link_id in self.agentic_memories
                                and neighbor_count < k
                            ):
